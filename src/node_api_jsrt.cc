...
namespace jsrtimpl {
  ...
  inline napi_status JsPropertyIdFromKey(JsValueRef key,
    ...
    if (keyType == JsString) {
      size_t length;
      CHECK_JSRT_EXPECTED(
        JsCopyStringUtf8(key, nullptr, 0, &length), napi_string_expected);

      std::vector<uint8_t> name;
      name.reserve(length + 1);
      CHECK_JSRT(JsCopyStringUtf8(key, name.data(), length + 1, &length));

      CHECK_JSRT(JsCreatePropertyIdUtf8(
  ...
  JsNameValueFromPropertyDescriptor(const napi_property_descriptor* p,
    ...
    if (p->utf8name != nullptr) {
      CHECK_JSRT(JsCreateStringUtf8(
        reinterpret_cast<const uint8_t*>(p->utf8name),
        strlen(p->utf8name),
...
napi_status napi_get_value_string_latin1(napi_env env,
  ...
  if (!buf) {
    ...
    JsErrorCode err = JsCopyStringUtf8(js_value, nullptr, 0, result);
    if (err != JsErrorInvalidArgument) {
    ...
    CHECK_JSRT_EXPECTED(
      JsCopyStringUtf8(
        js_value, reinterpret_cast<uint8_t*>(buf), bufsize, &copied),
